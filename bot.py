...
def sanitize_cookies(cookie_list):
    """Ensures cookie compatibility with Playwright/Chromium."""
    cleaned = []
...
def get_ai_reply(tweet_data):
    """Uses the Locked Super Prompt to generate contextual replies."""
    url = "https://openrouter.ai/api/v1/chat/completions"
    headers = {"Authorization": f"Bearer {AI_API_KEY}", "Content-Type": "application/json"}
    
    # THE 2026 SUPER PROMPT (STAYS THE EXACT SAME)
    ...
    try:
        ...
        with httpx.Client() as client:
            ...
            if resp.status_code == 200:
                ...
                # Remove accidental labels or quotes
...
async def run_bot():
    ...
    async with async_playwright() as p:
        # 1. SETUP: Large Viewport to prevent "outside viewport" errors
        ...
        # 2. SCRAPE: Capture content and metadata
        ...
        for tweet in tweet_elements:
            try:
                ...
                if unique_id and unique_id not in seen_ids and (now - tweet_time).total_seconds() < 900:
                    ...
                    candidates.append({"element": tweet, "data": {"text": text, "author": author, "media_desc": media_desc}, "id": unique_id})
            except: continue

        print(f"🎯 Fresh Candidates: {len(candidates)}")

        # 3. INTERACTION: The Resilient Loop
        for target in candidates[:3]:
            if target['id'] in seen_ids: continue
            
            reply_text = get_ai_reply(target['data'])
            if not reply_text: continue
            print(f"📝 Target: {target['data']['author']} | Strategy: {reply_text[:40]}...")

            try:
                # FORCE SCROLL TO CENTER: Fixes "Outside Viewport" error
                ...
                # HUMAN INPUT SEQUENCE: Fixes "Disabled Button" error
                ...
                for attempt in range(3):
                    ...
                    # Try hotkey first (standard human behavior)
                    ...
                    try:
                        ...
                        # Fallback: Programmatic click to bypass UI blockage
